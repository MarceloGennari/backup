...
bool evso; // This value sets whether we are using Edge3 (false) or Odometry (true)
std::vector<Odometry> odometry;
std::vector<EDGE3> edge3;
...
void sendCollection(vector <Pose3d_Node*> nodes, int id, string label) {
  vs_object_collection_t objs;
  size_t n = nodes.size();
  if (n > 1) {
    objs.id = id;
    ...
    vs_object_t poses[n];
    for (size_t i = 0; i < n; i++) {
      Pose3d_Node* node =  nodes[i];
      isam::Pose3d  pose;
      pose = (*node).value();

      Eigen::Quaterniond quat = euler_to_quat(pose.roll(), pose.pitch(), pose.yaw());
...
void add_cloud(unsigned int idx_x1) {
  ...
  // normalize the utime
  if(evso){
     utimeN = (float) idx_x1 / (float) odometry.size();
  } else {
    utimeN = (float) idx_x1/ (float) edge3.size();
  }
...
void add_pose(unsigned int idx_x1) {
  ...
  Eigen::Isometry3d poseI = Eigen::Isometry3d::Identity();
  Pose3d_Node* node = pg2_nodes.at(idx_x1);
  isam::Pose3d  pose;
  pose = (*node).value();
...
// Overloaded method to support Edge3
void add_odometry(unsigned int idx_x0, unsigned int idx_x1, const Pose3d&
  measurement, const Noise& noise) {
  // only add another node, if the current one does not exist
  Pose3d_Node* a1;
  evso = false;
  ...
  if (!node_exists(idx_x1)) {
    ...
    a1 = new Pose3d_Node();
    slam.add_node(a1);
    pg2_nodes.push_back(a1);
    node_mapping.push_back(std::make_pair(pg2_nodes.size()-1, idx_x1));
  } else {
    a1 = pg2_nodes.at(idx_x1);
  }

  Pose3d_Pose3d_Factor* o_a01 = new Pose3d_Pose3d_Factor(pg2_nodes.at(idx_x0), a1, measurement, noise);
  slam.add_factor(o_a01);
...
if(evso){
  ...
  add_cloud(0);
    ...
    Pose3d prior_origin(0,0,0,0,0,0);
    Pose3d_Node* a0 = new Pose3d_Node();
    Pose3d_Factor* p_a0 = new Pose3d_Factor(a0, prior_origin, SqrtInformation(100. * eye(6)));
    slam.add_node(a0);
    slam.add_factor(p_a0);

    // save the node
    pg2_nodes.push_back(a0);
    // associate the node id with the id of the prior (always 0)
    node_mapping.push_back(std::make_pair(pg2_nodes.size()-1, 0));
    // add to LCM
    add_pose(0);
    add_cloud(0);
}
  ...
  if (isam_trajectories.good()) {
    ...
    unsigned int idx_x0 = std::stoi(strs.at(1)), idx_x1 = std::stoi(strs.at(2));
        ...
        odometry.push_back({line_num, "ODOMETRY", idx_x0, idx_x1, measurement, SqrtInformation(sqrtinf)});
          ...
          // EDGE3 41 42 0.119541 3.57346e-05 1.51072e-32 0 0 -0.000420239 10 0 0 0 0 0 10 0 0 0 0 10 0 0 0 100 0 0 100 0 25
          /**********
           * x,y,z are pretty self-explanatory
           * t1 is the roll
           * t2 is the pitch
           * t3 is the yaw
           * *******/
          if(strs.at(0).compare("EDGE3") == 0){
        evso = false;
        unsigned int idx_x0 = std::stoi(strs.at(1)), idx_x1 = std::stoi(strs.at(2));
        double x =std::stod(strs.at(3)), y = std::stod(strs.at(4)), z = std::stod(strs.at(5)),
               t1 = std::stod(strs.at(6)), t2 = std::stod(strs.at(7)), t3 = std::stod(strs.at(8)),
               ixx = std::stod(strs.at(10)), ixy = std::stod(strs.at(11)), ixz = std::stod(strs.at(12)), ixt1 = std::stod(strs.at(13)), ixt2 = std::stod(strs.at(14)), ixt3 = std::stod(strs.at(15)),
               iyy = std::stod(strs.at(16)), iyz = std::stod(strs.at(17)), iyt1 = std::stod(strs.at(18)), iyt2 = std::stod(strs.at(19)), iyt3 = std::stod(strs.at(20)),
               izz = std::stod(strs.at(21)), izt1 = std::stod(strs.at(22)), izt2 = std::stod(strs.at(23)), izt3 = std::stod(strs.at(24)),
               it1t1 = std::stod(strs.at(25)), it1t2 = std::stod(strs.at(26)), it1t3 = std::stod(strs.at(27)),
               it2t2 = std::stod(strs.at(28)), it2t3 = std::stod(strs.at(29)),
               it3t3 = std::stod(strs.at(30)); // There is an empty space as a 9th char
        MatrixXd sqrtinf(6,6);
        sqrtinf <<
            ixx, ixy, ixz, ixt1, ixt2, ixt3,
            0 , iyy, iyz, iyt1, iyt2, iyt3,
            0 ,  0 , izz, izt1, izt2, izt3,
            0 ,  0 ,  0 , it1t1, it1t2, it1t3,
            0 ,  0 ,  0 ,   0  , it2t2, it2t3,
            0 ,  0 ,  0 ,   0  ,   0  , it3t3;
        Pose3d measurement(x,y,z,t3,t2,t1);

        // add to a vector of odometries
        edge3.push_back({line_num, "EDGE3", idx_x0, idx_x1, measurement, SqrtInformation(sqrtinf)});
      }
      ++line_num;
    }
...
void extract_maps(const std::vector<int> start, const std::vector<int> end) {
  ...
  for(size_t i = 0u; i<start.size(); ++i) {
    ...
    Pose2d_Node* node = pg1_nodes[node_mapping[start[i]+j].first];
      ...
      if(!evso && j%1 == 0 /* Just to compute one every x point clouds */){
    Pose3d_Node* node = pg2_nodes[node_mapping[start[i]+j].first];
        isam::Pose3d  pose;
        pose = (*node).value();
      ...
      // compute the transformation between the cloud and world(local)
      Eigen::Affine3d T_world_curr_pose = Eigen::Affine3d::Identity();
      T_world_curr_pose.translation() << pose.x(), pose.y(), pose.z();
...
void process_odometry() {
    ...
    slam.print_stats();
    sendCollection(pg2_nodes, obj_coll_id, "Poses");
    cin.get();
        }
    }
...
void getPoses(vector<Pose2d_Node*> &nodes){
    std::ofstream posesFile("poses.txt");

    for(int i = 0; i<nodes.size(); i++){
        double x = nodes[i]->value().x(), y = nodes[i]->value().y(), t = nodes[i]->value().t();
        posesFile << x << " " << y << " " << t << std::endl;
    }

    posesFile.close();
}

void getPoses(vector<Pose3d_Node*> &nodes){
    std::ofstream posesFile("poses.txt");

    // Notice that the file is ordered as yaw, pitch, roll
    for(int i = 0; i<nodes.size(); i++){
        double x = nodes[i]->value().x(), y = nodes[i]->value().y(), z = nodes[i]->value().z(), roll = nodes[i]->value().roll(), yaw = nodes[i]->value().yaw(), pitch =nodes[i]->value().pitch();
        posesFile << x << " " << y << " " << z << " " << yaw << " " << pitch << " " << roll << std::endl;
    }
